...
int main() {
    ...
    */
    ...
    int colortext;
    // Попробуем реализовать игру. 

    // задаём название консольного окна
    system("A console mini game app!");

    // получаем дескриптор окна для обращения к консоли
    HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);


      // устанавливаем взаимодействие с русскими буквами. Но использовать неизвестно будем ли.
    SetConsoleCP(1251);         // установка кодовой страницы win-cp 1251 в поток ввода
    SetConsoleOutputCP(1251);   // установка кодовой страницы win-cp 1251 в поток вывода

    cout << "Выберите цвет текста! Список всех цветов для текста: " << endl << "1: Green" << endl << "2: Red" << endl << "3: Cyan" << endl;
    cin >> colortext;

    int colortextselected = 0;
    string testingtext = "Теперь это цвет текста!!";
    while (colortextselected != 1)
    {
        switch (colortext)
        {
        case (1):
            SetConsoleTextAttribute(hStdOut, FOREGROUND_GREEN);
            cout << testingtext << endl;
            colortext = 1;
            colortextselected = 1;
            system("pause");
            break;
    ...
    cout << "Настроить цвет консоли! Введите значение из предложенного списка:" << endl << "1: Green" << endl << "2: Red" << endl << "3: Cyan" << endl;
    int consolecolor, consolecolorselected = 0;
    cin >> consolecolor;
    string testingtextbackground = "Теперь фон имеет этот цвет!";

    while (consolecolorselected != 1)
    {
        switch (consolecolor)
        {
        case (1):
        ...
        case (2):
            ...
            break;
        case(3):
            SetConsoleTextAttribute(hStdOut, BACKGROUND_BLUE | BACKGROUND_GREEN | BACKGROUND_INTENSITY);
    ...
    system("CLS");
    cout << testingtextbackground << endl;
    system("Pause");
